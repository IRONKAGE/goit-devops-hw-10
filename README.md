...
- **Secure by Default:** Пароль адміністратора генерується автоматично через провайдер `random_password`, прихований у консольному виводі (`sensitive`) та готовий до безпечної інтеграції з AWS Secrets Manager або External Secrets Operator.
...
| `multi_az` | `bool` | Ввімкнути резервування у кількох зонах доступності (для звичайного RDS). Aurora ігнорує це, оскільки розподіляється по 3 AZ автоматично. | Ні |
| `allocated_storage`| `number`| Обсяг пам'яті в ГБ (наприклад, `20`). Застосовується переважно для звичайного RDS. | Ні |
| `storage_type` | `string` | Тип диска (наприклад, `gp3` або `io1`). | Ні |
| `db_name` | `string` | Назва початкової бази даних, яка буде автоматично створена. | Ні |
| `username` | `string` | Ім'я головного користувача (майстер-користувача) бази даних. | Ні |
...
*Примітка: Aurora за своєю природою розподіляє та реплікує дані у трьох зонах доступності (Multi-AZ) автоматично, тому змінна `multi_az` для неї не має значення.*
...
> ```
>
> *Примітка: команда `terraform destroy` видалить лише ресурси, створені цим конфігом. Ваш S3-бакет зі стейтом та DynamoDB таблиця (якщо вони використовуються) залишаться недоторканими, щоб уникнути проблеми "курки та яйця". Модуль налаштований з `skip_final_snapshot = true` для швидкого та безперешкодного видалення БД під час тестування.*
